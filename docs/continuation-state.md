...
Latest local progress: the MathorCup 2023 B review has been converted into a reusable rail/timetable route. The repository now has a Type I archetype, a rail-timetable operation card, stricter model-plan/implementation/review gates, a runnable rail-timetable demo pipeline, and a compiled 10-page `paper/rail_demo.tex` example.
...
1. Strengthen the quality gate so the artifact ledger cannot pass while `reviews/review-current.md` is empty or still marked needs-revision.
2. Add automated checks for required artifacts by problem type, beginning with Type I rail/timetable cases.
